...
if [ -d dist ]; then
  ...
  docker cp $(docker create --rm ruby-plugin-debian:latest sleep 0):/${GEM_NAME}.tar.gz ./dist/debian_linux_amd64.tar.gz
  ...
  docker cp $(docker create --rm ruby-plugin-alpine:latest sleep 0):/${GEM_NAME}.tar.gz ./dist/alpine_linux_amd64.tar.gz
  ...
  docker cp $(docker create --rm ruby-plugin-centos:latest sleep 0):/${GEM_NAME}.tar.gz ./dist/centos_linux_amd64.tar.gz
  ...
  if [ -n "$package" ]; then
    ...
    cd dist || exit
    sha512_file="sha512-checksums.txt"
